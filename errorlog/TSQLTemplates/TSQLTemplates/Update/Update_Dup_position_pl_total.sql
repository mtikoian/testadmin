...
/* ----------------------------------------------------------------------------------------------------
 ...
 File Name				:Update_Dup_position_pl_total
...
IF OBJECT_ID('tempdb..#tempposition_pl_total') IS NOT NULL
	DROP TABLE #tempposition_pl_total;

--check if temp table exists         
IF OBJECT_ID('tempdb..#tbposition_pl_total') IS NOT NULL
	DROP TABLE #tbposition_pl_total

CREATE TABLE #tempposition_pl_total (
	Id INT identity(1, 1) NOT NULL PRIMARY KEY nonCLUSTERED
	,new_instr_id CHAR(16) NOT NULL
	,old_instr_id VARCHAR(16) NOT NULL
	);
	CREATE CLUSTERED INDEX nix_1 ON #tempposition_pl_total
(
       new_instr_id,
       old_instr_id
)

CREATE TABLE #tbposition_pl_total (
	RCD_NUM INT NOT NULL PRIMARY KEY
	,old_instr_id CHAR(16) NOT NULL
	);

INSERT INTO #tempposition_pl_total
...
INSERT INTO #tbposition_pl_total
select rcd_num, instr_id
...
--SELECT *
--FROM #tempposition_pl_total

--SELECT *
--FROM #tbposition_pl_total

--SELECT rcd_num
--	,tb.old_instr_id
--	,tmp.new_instr_id
--FROM #tempposition_pl_total tmp
--INNER JOIN #tbposition_pl_total tb ON tmp.old_instr_id = tb.old_instr_id
BEGIN TRY
	WHILE 1 = 1
	BEGIN
		BEGIN TRANSACTION;

		UPDATE TOP (@Updatebatchsize) pl
		SET pl.instr_id = tbl.new_instr_id
		--select instr_id  
		FROM dbo.position_pl_total pl
		INNER JOIN (
			SELECT rcd_num
				,tb.old_instr_id
				,tmp.new_instr_id
			FROM #tempposition_pl_total tmp
			INNER JOIN #tbposition_pl_total tb ON tmp.old_instr_id = tb.old_instr_id
			) tbl ON tbl.RCD_NUM = pl.rcd_num
			and tbl.old_instr_id = pl.instr_id;
