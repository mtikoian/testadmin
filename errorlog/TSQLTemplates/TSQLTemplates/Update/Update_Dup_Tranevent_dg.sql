...
/* ----------------------------------------------------------------------------------------------------
 ...
 File Name				:Update_Dup_Tranevent_dg
...
IF OBJECT_ID('tempdb..#temptranevent_dg') IS NOT NULL
	DROP TABLE #temptranevent_dg;

--check if temp table exists         
IF OBJECT_ID('tempdb..#tbtranevent_dg') IS NOT NULL
	DROP TABLE #tbtranevent_dg

CREATE TABLE #temptranevent_dg (
	Id INT identity(1, 1) NOT NULL PRIMARY KEY nonCLUSTERED
	,new_instr_id CHAR(16) NOT NULL
	,old_instr_id VARCHAR(16) NOT NULL
	);
	CREATE CLUSTERED INDEX nix_1 ON #temptranevent_dg
(
       new_instr_id,
       old_instr_id
)

CREATE TABLE #tbtranevent_dg (
	RCD_NUM INT NOT NULL PRIMARY KEY
	,old_instr_id CHAR(16) NOT NULL
	);

INSERT INTO #temptranevent_dg
...
--	,tb.old_instr_id
--	,tmp.new_instr_id
--FROM #temptranevent_dg tmp
--INNER JOIN #tbtranevent_dg tb ON tmp.old_instr_id = tb.old_instr_id
BEGIN TRY
	WHILE 1 = 1
	BEGIN
		BEGIN TRANSACTION;

		UPDATE TOP (@Updatebatchsize) tg
		SET tg.instr_id = tbl.new_instr_id
		--select instr_id  
		FROM dbo.tranevent_dg tg
		INNER JOIN (
			SELECT rcd_num
				,tb.old_instr_id
				,tmp.new_instr_id
			FROM #temptranevent_dg tmp
			INNER JOIN #tbtranevent_dg tb ON tmp.old_instr_id = tb.old_instr_id
			) tbl ON tbl.RCD_NUM = tg.rcd_num
			and tbl.old_instr_id = tg.instr_id;
