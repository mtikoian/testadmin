...
SET @schema_to_empty = N'cpd'
SET @sql = N''

-- drop all references to tables in the blat schema,
-- even FKs on tables in other schemas.
SELECT @sql = @SQL + 'IF  EXISTS ( SELECT 1	FROM sys.foreign_keys WHERE NAME = ' + '''' + QUOTENAME(k.[name]) + '''' + '	) BEGIN ' + N'	ALTER TABLE ' + QUOTENAME(OBJECT_SCHEMA_NAME(k.parent_object_id)) + '.' + QUOTENAME(OBJECT_NAME(k.parent_object_id)) + ' DROP CONSTRAINT ' + QUOTENAME(k.[name]) + '  end' + ';' + CHAR(13) + CHAR(10)
...
SELECT @sql = @sql + 'if exists (' + '   select 1 from information_schema.tables   where Table_Schema = ' + '''' + QUOTENAME(@schema_to_empty) + '''' + ' and Table_Name = ' + '''' + QUOTENAME([name]) + '''' + '     ) begin' + N'	DROP TABLE ' + QUOTENAME(@schema_to_empty) + '.' + QUOTENAME([name]) + ' end' + ';' + CHAR(13) + CHAR(10)
--@sql + N'	DROP TABLE ' + QUOTENAME(@schema_to_empty) + '.' + QUOTENAME([name]) + ';' + CHAR(13) + CHAR(10)
FROM sys.tables
