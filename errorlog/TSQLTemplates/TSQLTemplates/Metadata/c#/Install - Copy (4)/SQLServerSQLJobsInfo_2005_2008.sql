...
SELECT		J.name AS Name
			...
			,MIN(
					CASE WHEN JS.next_run_date = 0
						THEN NULL
						ELSE
							CONVERT(char(19), SUBSTRING(CAST(JS.next_run_date AS char(8)), 5, 2) + '/' + SUBSTRING(CAST(JS.next_run_date AS char(8)), 7, 2) + '/' + LEFT(CAST(JS.next_run_date AS char(8)), 4) + ' ' + 
								LEFT(REPLICATE('0', 6 - LEN(CAST(JS.next_run_time AS varchar(6)))) + CAST(JS.next_run_time AS varchar(6)), 2) + ':' +
								SUBSTRING(REPLICATE('0', 6-LEN(CAST(JS.next_run_time AS varchar(6)))) + CAST(JS.next_run_time AS varchar(6)), 3, 2) + ':' + 
								RIGHT(REPLICATE('0', 6-LEN(CAST(JS.next_run_time AS varchar(6)))) + CAST(JS.next_run_time AS varchar(6)), 2), 120)
					END
			    ) AS NextRunDate
...
FROM		msdb.dbo.sysjobs							J WITH (NOLOCK)
				LEFT JOIN sys.server_principals			P WITH (NOLOCK)		ON J.owner_sid		= P.sid
				LEFT JOIN msdb.dbo.sysjobhistory		H WITH (NOLOCK)		ON J.job_id			= H.job_id
																	       AND H.step_id		= 0
				LEFT JOIN msdb.dbo.sysjobschedules		JS WITH (NOLOCK)	ON J.job_id			= JS.job_id
				LEFT JOIN msdb.dbo.sysschedules			S WITH (NOLOCK)		ON JS.schedule_id	= S.schedule_id
																		   AND S.enabled		= 1
GROUP BY	J.Name
			,P.name 
			,J.Enabled
