...
create procedure dbo.p_Load_Table_Dimension_Date
	...
	@Start_Date_For_Id_Calculation		datetime	= null,
...
Written by: Don Clark on 2005/5/10
	This procedure loads T_DIMENSION_DATE table.

Updated by LByalsky on 2009/02/05
...
declare @Transaction_Name	varchar(255)
...
--	Capture the name of this stored procedure
set @procname = isnull(rtrim(object_name(@@procid)),'UNKNOWN_PROCEDURE')

--	Setup the Transaction name that this stored procedure will use
set @Transaction_Name = 'TRANS_' + @procname
...
begin try
	...
	if object_id('tempdb..#num1') is not null begin drop table #num1 end
	if object_id('tempdb..#num2') is not null begin drop table #num2 end
	if object_id('tempdb..#num3') is not null begin drop table #num3 end
	if object_id('tempdb..#ISO_WEEK') is not null	begin drop table #ISO_Week end
	if object_id('tempdb..#DIMENSION_DATE') is not null	begin drop table #Dimension_Date End
...
Update_T_Dimension_Date:
	...
	from dbo.Dimension_Date
		left join #Dimension_Date b on Dimension_Date.Dimension_Dt = b.Calendar_Date
...
-- Standard normal exit routine for all procedures, do not change
Normal_Exit:

select @Message = 'Normal End of Procedure '+@procname
print @Message


return 0

-- Standard error routine for all procedures, do not change
Error_Exit:

select @retcode = 0 - isnull( @Errorcode, 9999 )

select @ErrorMessage = isnull(@ErrorMessage+' - ','') +
	'Error in Procedure ' + isnull(@procname,'NULL') +
	', SQLErrorCode = ' +
	isnull(convert(varchar(20),@SQLErrorCode),'NULL') +
	', Errorcode = ' +
	isnull(convert(varchar(20),@Errorcode),'NULL') +
	', Rowcount = ' +
	isnull(convert(varchar(20),@rowcount),'NULL') +
	', retcode = ' +
	isnull(convert(varchar(20),@retcode),'NULL')
	-- Uncomment following line when using Cursor processing
	+ ', Fetch_Status = ' + isnull(convert(varchar(20),@fetch_status),'NULL')

if @@trancount > 0 and @@trancount > @trancount
	begin
	rollback transaction @Transaction_Name
	select @error = @@error , @rowcount = @@rowcount
	--	Process error in rollback,
	if (@@trancount > 0 and @@trancount > @trancount) or @error <> 0
		begin
		-- Perform full rollback if rollback of named transaction failed (Traps logic error)
