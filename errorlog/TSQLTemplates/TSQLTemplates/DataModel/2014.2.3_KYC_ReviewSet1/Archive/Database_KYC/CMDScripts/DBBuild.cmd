...
rem Dump the ServerList file
echo ********** Dumping the Server List **********
type %DBBuild_Server_List% >>%LogFile%
echo ********** Dumping the File List ************
type %Input_File_List% >>%LogFile% 
...
for /F "eol=# tokens=1-3* delims= " %%A IN (%DBBuild_Server_List%) DO (
    ...
    call DBBuild_SQLCmd.cmd
    if not !ERRORLEVEL!==0 GOTO ABORT
    echo ERRORLEVEL=!ERRORLEVEL! >> %LogFile%
    echo. >> %LogFile%
)    

rem 	Finally, delete all empty .out files 
rem 	Added 8/30/2012 SRM
pushd %LogDir% 
