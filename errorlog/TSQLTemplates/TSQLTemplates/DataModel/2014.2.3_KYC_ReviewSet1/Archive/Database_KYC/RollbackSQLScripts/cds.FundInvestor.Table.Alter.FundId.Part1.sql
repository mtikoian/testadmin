...
BEGIN TRY
	...
	IF NOT EXISTS (SELECT 1
		...
		BEGIN
			ALTER TABLE cds.FundInvestor ADD [FundIdOLD] int NULL;
			PRINT 'cds.FundInvestor FundIdOLD added'
			
			ALTER TABLE cds.FundInvestor ALTER COLUMN FundId int NULL;
			PRINT 'cds.FundInvestor FundId int NULL'
		END
	ELSE
		PRINT 'cds.FundInvestor FundIdOLD NOT added FundId NOT int NULL'
	...
	IF NOT EXISTS (SELECT 1
		       ...
		       WHERE  table_schema = 'cds'
		       		  ...
		       		  AND column_name = 'FundIdOLD')
		BEGIN
			ALTER TABLE cds.FundInvestor_Pending ADD [FundIdOLD] int NULL;
			PRINT 'cds.FundInvestor_Pending FundIdOLD added'
			
			ALTER TABLE cds.FundInvestor_Pending ALTER COLUMN FundId int NULL;
			PRINT 'cds.FundInvestor_Pending FundId int NULL'
		END
	ELSE
		PRINT 'cds.FundInvestor_Pending FundIdOLD NOT added FundId NOT int NULL'
