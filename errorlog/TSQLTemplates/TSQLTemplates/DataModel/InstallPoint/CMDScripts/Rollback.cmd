...
if %Rollback_File_List%zip==zip goto ABORT
if not exist %Rollback_File_List% goto ABORT    
...
for /F "eol=#" %%g in (%Rollback_File_List%) do (
	set Script=%%g
	set ErrMsg="%ROOTDIR%\sqlscripts\%%g does not exist!"
	if not exist %ROOTDIR%\sqlscripts\%%g goto ABORT
)
...
for /F "eol=#" %%g in (%Rollback_File_List%) do (
	...
	rem Status the user
	echo Executing %ROOTDIR%\sqlscripts\%%g
	echo Executing %ROOTDIR%\sqlscripts\%%g >> %LogFile%

	rem Execute the SQL script
	SQLCMD.exe -S %DBServer% -E -d %DBName% -i %ROOTDIR%\sqlscripts\%%g -b -w 2000 >> %LogFile% 2>&1
	if not !ERRORLEVEL!==0 GOTO ABORT
