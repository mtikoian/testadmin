...
EXEC msdb.dbo.sp_add_jobstep @job_id=@jobId, 
    @step_name=N'Backup Database', 
		...
		@os_run_priority=0, 
		@subsystem=N'CmdExec', 
		@command=N'SQLCMD -S $(ServerName) -b -E -d tempdb -Q "DECLARE @RC INT; EXEC @RC = master..xp_backup_database @database = ''$(DatabaseName)'', @filename = ''$(BackupLocation)\$(DatabaseName)_Adhoc.bak'', @with = ''COPY_ONLY'', @init = 1; IF @RC <> 0 RAISERROR(''Error running backup command'',16,1);"', 
