...
EXEC msdb.dbo.sp_add_jobstep @job_id=@jobId, 
    @step_name=N'Restore Database', 
		...
		@os_run_priority=0, 
		@subsystem=N'CmdExec', 
		@command=N'SQLCMD -S $(ServerName) -E -d tempdb -b -Q "ALTER DATABASE [$(DatabaseName)] SET SINGLE_USER WITH ROLLBACK IMMEDIATE;ALTER DATABASE [$(DatabaseName)] SET MULTI_USER;EXEC master..xp_restore_database @database= ''$(DatabaseName)'',@filename=''$(BackupLocation)\$(DatabaseName)_Adhoc.bak'', @with=''REPLACE'';"', 
