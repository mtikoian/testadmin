...
SET @JobStepCommand = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @Database + ' -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''ALL_DATABASES'', @Directory = ' + ISNULL('N''' + REPLACE(@BackupDirectory,'''','''''') + '''','NULL') + ', @BackupType = ''FULL'', @Verify = ''Y'', @CleanupTime = 168, @CheckSum = ''Y''" -b'
EXEC msdb.dbo.sp_add_jobstep
    @job_id = @JobId,
    @step_id = 2,
    @step_name = @JobStepName,
...
SET @JobStepName = 'Zip Backups Files';
SET @JobStepCommand = 'powershell ZipFiles.ps1 -Path ' + @BackupDir + ' -Mask "*.bak" -Recurse';
EXEC msdb.dbo.sp_add_jobstep
    @job_id = @JobId,
    @step_id = 3,
    @step_name = @JobStepName,
...
SET @JobStepName = 'Delete archived files'
SET @JobStepCommand = 'powershell DeleteArchivedFiles.ps1 -Path "' + @BackupDir + '" -Recurse'
EXEC msdb.dbo.sp_add_jobstep
    @job_id = @JobId,
    @step_id = 2,
    @step_name = @JobStepName,
