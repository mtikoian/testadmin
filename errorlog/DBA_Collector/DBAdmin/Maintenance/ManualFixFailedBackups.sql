...
SET @FullBackupThreshold = 6;
SET @DiffBackupThreshold = 3;
...
SELECT	name database_name,
		...
		CASE 
			...
			ELSE 0
		END needs_full,
		'EXEC master.dbo.DatabaseBackup @databases=' + QUOTENAME(name,'''') + ', @backupType=''FULL'', @CleanupTime=336, @Verify=''Y''' full_command,
		...
		CASE 
			...
			ELSE 0
		END needs_diff,
		'EXEC master.dbo.DatabaseBackup @databases=' + QUOTENAME(name,'''') + ', @backupType=''DIFF'', @CleanupTime=336, @Verify=''Y''' diff_command,
		...
		CASE 
			...
			ELSE 0
		END needs_log,
		'EXEC master.dbo.DatabaseBackup @databases=' + QUOTENAME(name,'''') + ', @backupType=''LOG'', @CleanupTime=48, @Verify=''Y''' log_command
FROM	LastBackups
