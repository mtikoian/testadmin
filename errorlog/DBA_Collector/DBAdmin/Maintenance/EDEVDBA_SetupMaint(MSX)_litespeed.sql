...
BEGIN -- Setup jobs --
  ...
  BEGIN -- EDEVDBA_DailyMaint --
    SET @JobName = 'EDEV DBA Daily Maintenance Job Litespeed (MSX)'
    ...
    EXEC msdb.dbo.sp_add_job
        @job_name = @JobName,
        @enabled = 1,
        @description = 'EDev DBA Daily Maintenance Litespeed (Master Server)',
        @owner_login_name = 'sa',
    ...
    EXEC msdb.dbo.sp_add_jobserver
	  ...
	  -- Remove step to create log directory
	  -- JF 4/3/2012
    ...
    EXEC msdb.dbo.sp_add_jobstep
	  ...
	  -- Commented since delete file command doesn't work with UNC paths
	  -- JF 4/3/2012
  ...
  BEGIN -- EDEVDBA_WeekendMaint --
    SET @JobName = 'EDEV DBA Weekend Maintenance Job Litespeed (MSX)'
    SET @JobID = NULL;
    ...
    EXEC msdb.dbo.sp_add_jobstep
	  ...
	  -- Commented since the delete command doesn't support UNC paths
	  -- JF 3/30/2012
    --SET @JobStepName = 'Output File Cleanup'
    --SET @JobStepCommand = 'cmd /q /c "For /F "tokens=1 delims=" %v In (''ForFiles /P "' + @OutputDir + '" /m *.txt /d -30 2^>^&1'') do if not "%v" == "ERROR: No files found with the specified search criteria." echo del "' + @OutputDir + '"\%v& del "' + @OutputDir + '"\%v"'
    --EXEC msdb.dbo.sp_add_jobstep
