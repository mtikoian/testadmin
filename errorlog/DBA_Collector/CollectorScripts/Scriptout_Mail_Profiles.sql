...
Declare @sql_profile nvarchar(4000);
...
IF NOT EXISTS(SELECT * FROM msdb.dbo.sysmail_profile WHERE  name = ''' + p.name + ''') 
  ...
  +
  '
  IF NOT EXISTS(SELECT 1 FROM msdb.dbo.sysmail_account WHERE  name = ''' + a.name + ''')
  BEGIN
...
IF EXISTS(SELECT 1
  ...
  END 
IF EXISTS(SELECT 1 FROM msdb.dbo.sysmail_account WHERE  name = ''' + a.name + ''')
  BEGIN
    EXECUTE msdb.dbo.sysmail_delete_account_sp @account_name = ''' + a.name + '''
  END
IF EXISTS(SELECT 1 FROM msdb.dbo.sysmail_profile WHERE  name = ''' + p.name + ''') 
  BEGIN
...
LEFT OUTER JOIN sys.credentials c    ON s.credential_id = c.credential_id
  ...
  --from ItemSplit  
  		...
  		DECLARE CURSOR_DB_Profile CURSOR FAST_FORWARD
		FOR   
			   WITH E01(N) AS (SELECT 1 UNION ALL SELECT 1 UNION ALL  
                    SELECT 1 UNION ALL SELECT 1 UNION ALL  
                    SELECT 1 UNION ALL SELECT 1 UNION ALL  
  ...
  from ItemSplit  
		...
		OPEN CURSOR_DB_Profile

		FETCH NEXT FROM CURSOR_DB_Profile
		INTO @sql_profile

		IF @@FETCH_STATUS = 0
			BEGIN
				PRINT ''
				PRINT '--Server has Database Profiles Enabled:  '		
			END
		...
		CLOSE CURSOR_DB_Profile
		DEALLOCATE CURSOR_DB_Profile