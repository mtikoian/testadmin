...
SET @SQL = @SQL +     ',S.backup_size / (SELECT COUNT(*) FROM msdb.dbo.backupmediafamily A WHERE S.media_set_id = A.media_set_id )  ' 
SET @SQL = @SQL +     ',S.user_name ' 
SET @SQL = @SQL +     ',S.name ' 
SET @SQL = @SQL +     ',F.physical_device_name ' 

IF LEFT(CAST(SERVERPROPERTY('ProductVersion') AS varchar(25)), CHARINDEX('.', CAST(SERVERPROPERTY('ProductVersion') AS varchar(25))) - 1) NOT IN ('8', '9')
	SET @SQL = @SQL +     ',S.compressed_backup_size ' 
ELSE
	SET @SQL = @SQL +     ',NULL ' 

SET @SQL = @SQL +     ',S.description ' 
...
SELECT		D.name + '<1>' +
				...
				ISNULL(L.BackupFileName, 'NULL') + '<13>' + 
				ISNULL(CAST(L.CompressedSize AS varchar(25)), 'NULL')
				 --+ '<14>' 
				--+
				--ISNULL(L.Description, 'NULL')
