use AdventureWorks2014;  -- 5 seconds
go 
...
begin try;
    ...
    exec sp_addpublication 
        ...
        @description = 'Transactional publication of AdventureWorks2014 database from Publisher ebdbalt002\jwf.',
        @status = 'active',                -- The subscribers will get data immediately on subscription
        @independent_agent = 'true ',      -- 
        @immediate_sync = 'false',          -- IMPORTANT - If set to true - a complete snapshot will be generated every time the snapshot agent starts. 
        ...
        @retention = 336,                  -- the default.  The time (in hours) that data is maintained in the distribution database.  336 = 14 days.
                                           -- Note:  @max_distretention in the distributor wins if it is less then @retention.  Important for Initialize from backup.
        ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        @queue_type = 'sql',
        @allow_initialize_from_backup = 'false',  -- If set to true - @retention will be respected at all times.  Steady state - this should be over kill.
