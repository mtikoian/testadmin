...
/* Make sure a mirroring end point does not already exist and then create one on the Principal */
IF Not Exists (SELECT 1 FROM sys.database_mirroring_endpoints)
	BEGIN
		CREATE ENDPOINT [Mirror_Default] --InstanceName/MachineName_MirrorEP is a good naming convention
			STATE = STARTED
			AS TCP (LISTENER_PORT = 5022, LISTENER_IP = ALL) --Specify Listener_IP if using a dedicated NIC for Mirroring
...
/* Create a login on the Principal for the account running the SQL Service on the Mirror. */
CREATE LOGIN [PrincipalInstance\MirrorServiceAccount] FROM WINDOWS;

/* Grant Connect on Principal Endpoint to Mirror Login */
GRANT CONNECT ON ENDPOINT::[Mirror_Default] TO [PrincipalInstance\MirrorServiceAccount]

/* Set the mirroring partner on the principal
*** Partner MUST be set on the Mirror FIRST ***/
ALTER DATABASE MyDB
SET PARTNER = 'TCP://MirrorFQDN:5023';

/* Set the amount of time with no ping response before failover occurs on principal */
ALTER DATABASE MyDB
