...
WHERE xs.name = 'ex_SOS_SCHEDULER_YIELD'
GO 
...
AS (
    ...
    FROM sys.dm_xe_session_targets xst
        INNER JOIN sys.dm_xe_sessions xs ON xst.event_session_address = xs.address
    WHERE xst.target_name = 'histogram' and xs.name = 'ex_SOS_SCHEDULER_YIELD'
)
,frames 
AS (
...
FROM levels l
    ...
    OUTER APPLY (SELECT plan_handle FROM sys.dm_exec_query_stats qs WHERE l.sql_handle = qs.sql_handle GROUP BY plan_handle) x
    OUTER APPLY sys.dm_exec_query_plan(x.plan_handle) qp